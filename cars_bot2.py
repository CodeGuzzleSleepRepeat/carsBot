...
def get_mes_by_client(manager, client, chat_id):
	...
	for a in arr:
		...
		words = a.split(';;')	
		try:
			print((datetime.date.today() - datetime.date(int('20' + words[2][6:8]), int(words[2][:2]), int(words[2][3:5]))).days)
			if (datetime.date.today() - datetime.date(int('20' + words[2][6:8]), int(words[2][:2]), int(words[2][3:5]))).days > 1:
				continue
...
def send_file(message):
	...
	while gl_flag[message['message']['chat']['id']] == 0:
		time.sleep(0.1)	
				...
				try:
					...
					for ch_id in chat_ids:
						...
						if str(message).find('query') == -1:	
							...
							gl_flag[message['message']['chat']['id']] = 0
							gl_flag2[message['message']['chat']['id']] = 0
							if str(message).find('username') > -1:
...
def get_mes_by_client(manager, client, chat_id):
	...
	for a in arr:
		...
		words = a.split(';;')	
		try:
			print((datetime.date.today(), datetime.date(int('20' + words[2][6:8]), int(words[2][:2]), int(words[2][3:5]))))
			if (datetime.date.today() - datetime.date(int('20' + words[2][6:8]), int(words[2][:2]), int(words[2][3:5]))).days > 1:
				continue
			if words[1].lower() == manager.lower() and str(words[3]) == str(client):
				send_message(chat_id, str(words[2]) + ': ' + str(words[0]))
